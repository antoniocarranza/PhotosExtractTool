...
def composeSQLUpdateCommand(masterId,newImagePath,newFileIsReference):
    return """
        UPDATE RKMaster
        SET
            m_oldImagePath='{imagePath}',
            m_oldFileIsReference={fileIsReference}
        WHERE
            modelId={modelId}
        ;
        """.format(imagePath=newImagePath, fileIsReference=newFileIsReference, modelId=masterId)
...
def main(argv):
    ...
    for idx,row in enumerate(rows):
        ...
        try:
            if alterTables:
                cursor.execute(composeSQLUpdateCommand(masterId,target,1))
        except:
